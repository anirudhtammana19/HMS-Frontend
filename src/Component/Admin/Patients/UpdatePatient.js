...
const UpdatePatient = ({ show, handleClose, setRefresh, patient }) => {
  ...
  const handleSave = () => {
    if (token) {
      axios
        .put(`http://localhost:8080/api/patient/updateprofile/${patient.patientId}`, formData, {
          headers: {
        ...
        .catch((err) => {
          ...
          message.error("Error editing Patient");
        });
    }
  };
